...
あらゆるものが攻撃ベクターとなりうるため、その対策を怠らない。
* 入力データ：ファイル名を利用した任意コード実行など
* 認証の不要なサービス：誰でもつなげるならDoSをくらう
* 認証を要するサービス：権限を付与された古いアカウントが残っている
* 出力データ：パスワードがすでに使用済みなどの余計な情報開示
* デバッグ情報：クラッシュしたウェブデータとスタックトレース

## 44  ものの名前
ものの名前は意図と確信を表現する大事なもの。
文化によっても意味合いが異なってくる。チームのコミュニケーション（ペアプログラミング）や用語集を貼り付けることで浸透していき、しばらくすると用語集は独自の進化を遂げ、専門用語を使って首尾一貫性のある形で迅速な意思疎通が行えるようになる。

しかし、最適な名前は時とともに移り変わるもの。問題を見つけたら、3章で示したようにその場で修正しよう。